...
// UpdateBoards takes a pointer to an initial Board object and a number of steps parameter, as well as a boolean to represent whether to
...
// It takes a boolean parameter corresponding to whether to run
// the update in parallel (if parameter = true) or serially (if it's false).
...
func (b *Board) Diffuse(isParallel bool) {
	...
	} else { // we are running a serial simulation
...
//It takes a pointer to a PRNG object as input, which it uses for the
